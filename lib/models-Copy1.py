...
class XGBoost(Model):
    ...
    def input(self, preprss_object):
        # generating random file name to dump preprocessing
        
#         self.preprocess_p_file = preprss_file_name
        self.preprocess_object= preprss_object
#         self.load_preprocess_object()
#         self.save_preprocess_object()
        self.model_results=None    
        self.train()
...
#         self.model_results= self.rmse(self.preprocess_object.get_test_x(), self.preprocess_object.get_test_y())
#         print("Result on validation data: ", self.model_results)
...
class NN_with_EntityEmbedding(Model):
    ...
    def input(self, preprss_object):
        ...
        self.model_results=None
        self.nb_epoch = 10
        self.checkpointer = ModelCheckpoint(filepath="saved_models/best_model_weights.hdf5", verbose=1, save_best_only=True)
        self.checkpointer = TensorBoard(log_dir="logs/{}".format(time()))
        self.model_architecture= 'saved_models/model_architecture.json'
        self.model_weights= "saved_models/best_model_weights.hdf5"
    ...
    def __build_keras_model(self, input_dim, output_dim):
        ...
        with open(self.model_architecture, 'w') as f:
            f.write(self.model.to_json())

    def _val_for_fit(self, val):
        val = numpy.log(val+1) / self.max_log_y

        return val

    def _val_for_pred(self, val):
        ans = numpy.exp(val * self.max_log_y) -1
        return ans
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        self.model.fit(self.preprocessing(X_train), self._val_for_fit(y_train),
                       ...
                       nb_epoch=self.nb_epoch, batch_size=10,
    ...
    def load_model(self, model_archi, model_weights):
        self.model_architecture= model_archi
        self.checkpointer = ModelCheckpoint(filepath=model_weights, verbose=1, save_best_only=True)
        self.model_weights= model_weights

        # Model reconstruction from JSON file
        with open(self.model_architecture, 'r') as f:
    ...
    def get_model(self):
        return [self.model_architecture, self.model_weights]
...
class NN2_with_EntityEmbedding(Model):  # not required right now
    ...
    def __build_keras_model(self, input_dim, output_dim):
        ...
        self.model.add(Dense(2000, init='uniform'))
        self.model.add(Activation('relu'))
        model.add(Dropout(0.2))
        self.model.add(Dense(1000, init='uniform'))
        self.model.add(Activation('relu'))
